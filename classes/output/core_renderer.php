        ...
        if ($sitefont != 'Moodle') {
            if ($sitefont == 'Roboto') {
                $output .= '<style>
                    @font-face {
                        font-family: "Roboto";
                        src: url("' . $CFG->wwwroot . '/theme/moove/fonts/Roboto-Regular.woff2") format("woff2");
                        font-weight: 400;
                        font-style: normal;
                        font-display: swap;
                    }
                    ...
                    @font-face {
                        font-family: "Roboto";
                        src: url("' . $CFG->wwwroot . '/theme/moove/fonts/Roboto-Medium.woff2") format("woff2");
                        font-weight: 500;
                        font-style: normal;
                        font-display: swap;
                    }
                    ...
                    @font-face {
                        font-family: "Roboto";
                        src: url("' . $CFG->wwwroot . '/theme/moove/fonts/Roboto-Bold.woff2") format("woff2");
                        font-weight: 700;
                        font-style: normal;
                        font-display: swap;
                    }
                    ...
                    @font-face {
                        font-family: "Roboto";
                        src: url("' . $CFG->wwwroot . '/theme/moove/fonts/Roboto-Italic.woff2") format("woff2");
                        font-weight: 400;
                        font-style: italic;
                        font-display: swap;
                    }
...
public function get_theme_topbanner_url() {
    $theme = \theme_config::load('moove');
    return $theme->setting_file_url('topbanner', 'topbanner');
}



public function get_theme_footerbg_url() {
    $theme = \theme_config::load('moove');
    return $theme->setting_file_url('footerbg', 'footerbg');
}
    ...
    $news = new \local_news\output\news_slider();
    $newsoutput = new \local_news\output\renderer($this->page, $this);
    return $newsoutput->render_news_slider($news);
    }


    /**
     * Override the export_for_template to inject admin role
     */
    public function render_from_template($templatename, $context) {
        ...
        // Nếu là admin thì luôn được
        if (is_siteadmin()) {
            $hasaccess = true;
        } else {
            // Lấy tất cả vai trò của user trong các context
            $roles = get_user_roles(\context_system::instance(), $USER->id, true);

            // Kiểm tra nếu user có role 'editingteacher' trong ít nhất 1 khóa học
            $sql = "SELECT ra.id
                      FROM {role_assignments} ra
                      JOIN {context} ctx ON ctx.id = ra.contextid
                     WHERE ra.userid = :userid
                       AND ra.roleid = :teacherrole
                       AND ctx.contextlevel = :coursectx";
            $params = [
                'userid' => $USER->id,
                'teacherrole' => 3, // 3 là ID mặc định của editingteacher
                'coursectx' => CONTEXT_COURSE
            ];
            $hasroleincourse = $DB->record_exists_sql($sql, $params);
        ...
        // Truyền biến vào context
        if (is_array($context)) {
            $context['hasaccessmenu'] = $hasaccess;
