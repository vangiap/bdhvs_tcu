...
function theme_moove_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = []) {
    ...
    // 🆕 Serve box images - Pattern: sliderboxX_Y_image (X=slide, Y=box)
    if ($context->contextlevel == CONTEXT_SYSTEM && preg_match("/^sliderbox[0-9]{1,2}_[0-9]{1,2}_image$/", $filearea) !== false) {
        return $theme->setting_file_serve($filearea, $args, $forcedownload, $options);
    }
