...
if ($ADMIN->fulltree) {
    ...
    */
    ...
    $setting = new admin_setting_configcolourpicker($name, $title, $description, '#ad0f0f');
    $setting->set_updatedcallback('theme_reset_all_caches');
    $page->add($setting);
    ...
    */
    $page = new admin_settingpage('theme_moove_frontpage', get_string('frontpagesettings', 'theme_moove'));
     ...
     // Cài đặt ảnh banner đầu trang (top banner)
    $name = 'theme_moove/topbanner';
    $title = get_string('topbanner', 'theme_moove');
    // $description = 'Ảnh banner hiển thị ở đầu trang (Top Banner)';
    $description = get_string('Ảnh banner hiển thị ở đầu trang (Top Banner)', 'theme_moove');
    $setting = new admin_setting_configstoredfile($name, $title, $description, 'topbanner');
    $page->add($setting);
    ...
    if ($slidercount) {
        for ($sliderindex = 1; $sliderindex <= $slidercount; $sliderindex++) {
            ...
            // 🆕 Phụ đề slide (slide subtitle)
            $name = 'theme_moove/slidesubtitle' . $sliderindex;
            $title = get_string('slidesubtitle', 'theme_moove');
            $description = get_string('slidesubtitledesc', 'theme_moove');
            $setting = new admin_setting_configtext($name, $title, $description, '', PARAM_TEXT);
            $page->add($setting);

            // 🆕 Số lượng boxes cho slide này
            $name = 'theme_moove/sliderboxcount' . $sliderindex;
            $title = get_string('sliderboxcount', 'theme_moove');
            $description = get_string('sliderboxcountdesc', 'theme_moove');
            $default = 0;
            $options = [];
            for ($i = 0; $i < 7; $i++) {
                $options[$i] = $i;
            ...
            // 🆕 Thêm fields cho từng box
            $sliderboxcount = get_config('theme_moove', 'sliderboxcount' . $sliderindex);
            if (!$sliderboxcount) {
                $sliderboxcount = 0;
            }
            ...
            if ($sliderboxcount) {
                for ($boxindex = 1; $boxindex <= $sliderboxcount; $boxindex++) {
                    // Tiêu đề cho nhóm boxes
                    if ($boxindex == 1) {
                        $setting = new admin_setting_heading(
                            'sliderboxes_heading_' . $sliderindex,
                            '',
                            '<strong>Boxes:</strong>'
                        );
                        $page->add($setting);
                    ...
                    // Hình ảnh box
                    $fileid = 'sliderbox' . $sliderindex . '_' . $boxindex . '_image';
                    $name = 'theme_moove/' . $fileid;
                    $title = get_string('sliderboximage', 'theme_moove', $boxindex . '');
                    $description = get_string('sliderboximagedesc', 'theme_moove');
                    $opts = ['accepted_types' => ['.png', '.jpg', '.gif', '.webp', '.tiff', '.svg'], 'maxfiles' => 1];
                    $setting = new admin_setting_configstoredfile($name, $title, $description, $fileid, 0, $opts);
                    $page->add($setting);

                    // Tiêu đề box
                    $name = 'theme_moove/sliderboxtitle' . $sliderindex . '_' . $boxindex;
                    $title = get_string('sliderboxtitle', 'theme_moove', $boxindex . '');
                    $description = get_string('sliderboxdesc', 'theme_moove');
                    $setting = new admin_setting_configtext($name, $title, $description, '', PARAM_TEXT);
                    $page->add($setting);

                    // Text nút bấm
                    $name = 'theme_moove/sliderboxbtn' . $sliderindex . '_' . $boxindex;
                    $title = get_string('sliderboxbuttontext', 'theme_moove', $boxindex . '');
                    $description = get_string('sliderboxbuttontextdesc', 'theme_moove');
                    $setting = new admin_setting_configtext($name, $title, $description, '', PARAM_TEXT);
                    $page->add($setting);

                    // Link nút bấm
                    $name = 'theme_moove/sliderboxlink' . $sliderindex . '_' . $boxindex;
                    $title = get_string('sliderboxbuttonlink', 'theme_moove', $boxindex . '');
                    $description = get_string('sliderboxbuttonlinkdesc', 'theme_moove');
                    $setting = new admin_setting_configtext($name, $title, $description, '', PARAM_URL);
                    $page->add($setting);
                }
            }

            // 🆕 Center logo upload for this slide
            $fileid = 'slidercenterlogo' . $sliderindex;
            $name = 'theme_moove/' . $fileid;
            $title = get_string('slidercenterlogo', 'theme_moove', $sliderindex . '');
            $description = get_string('slidercenterlogodesc', 'theme_moove');
    ...
    */
    ...
    // Cài đặt ảnh footer (footer bg)
    $name = 'theme_moove/footerbg';
    $title = get_string('footerbg', 'theme_moove');
    // $description = 'Ảnh banner hiển thị ở đầu trang (Top Banner)';
    $description = get_string('Ảnh nền footer', 'theme_moove');
    $setting = new admin_setting_configstoredfile($name, $title, $description, 'footerbg');
    $page->add($setting);
